...
try:
    ...
    plt.title('B站 (58238) 风速垂直结构随时间演变')
    plt.xlabel('时间 (UTC)')
    plt.ylabel('高度 (m)')
...
try:
    target_time = df_cleaned['time'].iloc[0]
    ...
    if df_time.empty:
        ...
        plt.title(f'B站 (58238) {target_time.strftime("%Y-%m-%d %H:%M")} 垂直廓线')
        fig.tight_layout()
        plt.show()
...
# ✅ 保存清洗后的数据
output_csv = 'b站点_风廓线雷达_清洗后数据.csv'
